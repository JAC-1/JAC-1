...
> Self-hosted workflow automation stack showcasing Docker Compose orchestration, a RAG pipeline, and a custom web frontend.

Runs four services together: n8n for visual workflow automation, Caddy as a reverse proxy and static file server, Qdrant as a vector database, and Redis as a cache/queue backend — all defined in a single `docker-compose.yml`.

Includes exported n8n workflows for a RAG (Retrieval-Augmented Generation) document pipeline and a textbook Q&A assistant. A lightweight HTMX frontend served by Caddy handles file uploads and chat, feeding into n8n webhooks.

**Stack:** n8n · Docker Compose · Caddy · Qdrant · Redis · HTMX

[→ n8n-caddy-docker](https://github.com/JAC-1/n8n-caddy-docker)
...
### n8n 自動化スタック
> Docker Compose によるオーケストレーション、RAG パイプライン、カスタム Web フロントエンドを組み合わせた自己ホスト型ワークフロー自動化スタックです。

4つのサービスを連携させています：ビジュアルワークフロー自動化の n8n、リバースプロキシ兼静的ファイルサーバーの Caddy、ベクターデータベースの Qdrant、キャッシュ/キューの Redis — すべて単一の `docker-compose.yml` で定義。

RAG（検索拡張生成）ドキュメントパイプラインと教科書 Q&A アシスタント用の n8n ワークフローエクスポートを含みます。Caddy が配信する軽量 HTMX フロントエンドがファイルアップロードとチャットを処理し、n8n の Webhook に接続します。

**技術スタック：** n8n · Docker Compose · Caddy · Qdrant · Redis · HTMX
